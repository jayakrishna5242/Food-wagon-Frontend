                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {orders.map((order) => (
                      <motion.div 
                        ...
                        className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex flex-col h-full hover:shadow-md transition-all group cursor-pointer relative overflow-hidden"
                      >
                        <div className="flex justify-between items-start mb-3">
                          <h3 className="text-lg font-extrabold text-dark leading-tight group-hover:text-primary transition-colors line-clamp-1">
                            {order.restaurantName || "The Sizzling Grill"}
                          </h3>
                          <div className="flex items-center gap-1 bg-gray-50 px-2 py-1 rounded-lg">
                            <Star className={`w-3 h-3 ${order.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} />
                            <span className="text-[10px] font-black text-dark">{order.rating || '--'}</span>
                          </div>
                        </div>

                        <p className="text-xs text-gray-500 mb-4 line-clamp-2 flex-grow leading-relaxed">
                          {order.items?.map(item => `${item.name} x${item.quantity}`).join(', ') || "Order details not available."}
                        </p>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-gray-50 mt-auto">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-black text-dark">₹{order.totalAmount}</span>
                          </div>
                          ...
                          <div className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${
                            ...
                            <div className={`w-1.5 h-1.5 rounded-full ${order.status === 'DELIVERED' ? 'bg-green-600' : 'bg-orange-600'}`}></div>
                            {order.status}
                          </div>

                          <div className="flex items-center gap-1">
                             <button className="p-1.5 text-gray-300 hover:text-yellow-400 transition-colors">
                               <Star className="w-4 h-4" />
                             </button>
                             <button className="p-1.5 text-gray-300 hover:text-blue-500 transition-colors">
